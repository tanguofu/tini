...
#if TINI_MINIMAL
#define PRINT_FATAL(...)               \
	fprintf(tini_stderr, __VA_ARGS__); \
	fprintf(tini_stderr, "\n");
...
#define PRINT_FATAL(...)                                                            \
	{                                                                               \
		fprintf(tini_stderr, "[%s FATAL tini (%i)] ", get_time(time(0)), getpid()); \
		fprintf(tini_stderr, __VA_ARGS__);                                          \
		fprintf(tini_stderr, "\n");                                                 \
		fflush(tini_stderr);                                                        \
	}
#define PRINT_WARNING(...)                                                          \
	if (verbosity > 0)                                                              \
	{                                                                               \
		fprintf(tini_stderr, "[%s WARN  tini (%i)] ", get_time(time(0)), getpid()); \
		fprintf(tini_stderr, __VA_ARGS__);                                          \
		fprintf(tini_stderr, "\n");                                                 \
		fflush(tini_stderr);                                                        \
	}
#define PRINT_INFO(...)                                                             \
	if (verbosity > 1)                                                              \
	{                                                                               \
		fprintf(tini_stderr, "[%s INFO  tini (%i)] ", get_time(time(0)), getpid()); \
		fprintf(tini_stderr, __VA_ARGS__);                                          \
		fprintf(tini_stderr, "\n");                                                 \
		fflush(tini_stderr);                                                        \
	}
#define PRINT_DEBUG(...)                                                       \
	if (verbosity > 2)                                                         \
	{                                                                          \
		fprintf(stdout, "[%s DEBUG tini (%i)] ", get_time(time(0)), getpid()); \
		fprintf(stdout, __VA_ARGS__);                                          \
		fprintf(stdout, "\n");                                                 \
	}
#define PRINT_TRACE(...)                                                       \
	if (verbosity > 3)                                                         \
	{                                                                          \
		fprintf(stdout, "[%s TRACE tini (%i)] ", get_time(time(0)), getpid()); \
		fprintf(stdout, __VA_ARGS__);                                          \
		fprintf(stdout, "\n");                                                 \
	}
